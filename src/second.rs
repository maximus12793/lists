...
impl<T> Iterator for IntoIter<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        // Get fields of tuple struct by number
        self.0.pop()
    }
}
...
impl<T> List<T> {
    ...
    pub fn push(&mut self, elem: T) {
        let new_node = Box::new(Node {
            elem: elem,
            next: self.head.take(), // Steal value out of borrow.
        });

        self.head = Some(new_node);
    }

    // TODO: implement pop_node(&mut self) -> Link from https://rust-unofficial.github.io/too-many-lists/first-drop.html
